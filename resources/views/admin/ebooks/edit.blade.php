...
@section('page-content')
  <div class="container-fluid">
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Edit Ebook</h1>
    ...
    <div class="card shadow mb-4">
      <div class="card-body">
        ...
        <div class="row">
          ...
          <div class="col-md-8">
            <form class="user mt-4" method="POST" action="/admin/ebooks/{{ $ebook->slug }}"
              enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <div class="form-group">
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                  name="title" placeholder="Enter Title ebook" value="{{ old('title', $ebook->title) }}" required>
                @error('title')
              ...
              <div class="input-group mb-3">
                ...
                <select class="custom-select" id="category_id" name="category_id">
                  @foreach ($categories as $category)
                    @if (old('category_id', $ebook->category_id) == $category->id)
                      <option value="{{ $ebook->category->id }}" selected>{{ $ebook->category->name }}</option>
                    @else
              ...
              <div class="custom-file col-sm-12 mb-4">
                <input type="file"
                  ...
                  id="image" name="image">
                <label class="custom-file-label py-2" for="image">Choose New Photo</label>
              </div>
              <div class="form-check mb-4">
                @if ($ebook->download)
                  <input class="form-check-input" type="checkbox" value="active" id="download" name="download" checked>
                @else
                  <input class="form-check-input" type="checkbox" value="active" id="download" name="download">
                @endif
              ...
              @enderror
              <div class="form-group">
                <input id="body" type="hidden" name="body" value="{{ @old('body', $ebook->body) }}">
